...
function [invA] = matrixInverse (A)
  ...
  A = [transpose(A);eye(n)];
  A = transpose(A); %this should result in the augmented matrix 
  %with the identity
  %NOw we are gonna do operations on identity part too
  ...
  I = A(:,[n+1:2*n]); %only be the identity portion that was changed
  [A2, Pt,L,U] = matrixFactorization(A);
  invA = backward_sub (U, transpose(Pt)*I);
  end
